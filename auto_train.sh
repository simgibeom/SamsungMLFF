...
common = dict(ediff=0.4, fdiff=0.4, ediff_tot=0.4, process_group=mpi_init())
kernel_kw = dict(lmax=3, nmax=3, exponent=4, cutoff=4.0)

ML_calc = ActiveCalculator(calculator=None,
                           logfile='active-$i-04.log',
                           pckl='model-$i-04.pckl/',
                           tape='model-$i-04.sgpr',
                           kernel_kw=kernel_kw,
                           **common)
...
calc = ActiveCalculator(covariance='model-$i-04.pckl',
                        calculator=None,
                        logfile='active-$i-04-p.log',
                        pckl='model-$i-04-p.pckl/',
                        tape='model-$i-04-p.sgpr',
                        kernel_kw=kernel_kw,
                        **common)
t1 = time.time()
error_list = []
