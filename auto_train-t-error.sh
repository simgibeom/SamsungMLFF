sub=16 # this is not necessary to training test, so enter arbitrary integer.
cut=6.0  # this value must be float
range=`tail -1 list_number.txt`
core=16
...
for i in $(seq 0 $range)  ; do
cd task_subset-$i/re-01/re-001/re-0$old_diff/
mkdir re-0$diff
cp subset-$i.traj v_Si_$sub.traj re-0$diff/
echo "  "
echo " ##### Training the model-$i using the subset-$i ... ##### "
